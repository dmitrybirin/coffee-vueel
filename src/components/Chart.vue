...
export default {
	...
	methods: {
		...
		getIndexFromAngle(angle) {
			if (angle > 2 * Pi - Pi / this.dataAngles.length) {
				...
				const angleDiff = this.dataAngles.map(da => Math.abs(da - angle))
	...
	mounted() {
		...
		this.dataAngles = this.dataLabels.map((label, idx) => this.chart.scale.getIndexAngle(idx))
	},
}
</script>
